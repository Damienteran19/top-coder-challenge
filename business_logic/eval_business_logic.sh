...
# Check if ./run_business_logic.sh exists
if [ ! -f "run_business_logic.sh" ]; then
    echo "❌ Error: run_business_logic.sh!"
    echo "Please create a run_business_logic.sh script that takes three parameters:"
    echo "  ./run_business_logic.sh <trip_duration_days> <miles_traveled> <total_receipts_amount>"
    echo "  and outputs the reimbursement amount"
    exit 1
fi

# Make ./run_business_logic.sh executable
chmod +x run_business_logic.sh
...
for ((i=0; i<num_cases; i++)); do
    ...
    if script_output=$(./run_business_logic.sh "$trip_duration" "$miles_traveled" "$receipts_amount" 2>/dev/null); then
        ...
        error_msg=$(./run_business_logic.sh "$trip_duration" "$miles_traveled" "$receipts_amount" 2>&1 >/dev/null | tr -d '\n')
...
echo "  2. Ensure your ./run_business_logic.sh outputs only a number"

...
def engineer_features(df):
    ...
    # --- Receipt Features ---
    df['log_receipts'] = np.log(df['total_receipts_amount'])
    ...
    #df['under_200_miles'] = (df['miles_traveled'] < 200).astype(int)
    #df['high_mileage'] = (df['miles_traveled'] > 800).astype(int)
    ...
    #df['miles_x_receipts_x_duration'] = df['miles_traveled'] * df['total_receipts_amount'] * df['trip_duration_days']
    #df['(miles_x_duration)_per_receipt'] = (df['miles_traveled'] * df['trip_duration_days']) / df['total_receipts_amount'].replace(0, 1)
    #df['(miles_x_receipts)_per_duration'] = (df['miles_traveled'] * df['total_receipts_amount']) / df['trip_duration_days'].replace(0, 1)
    #df['(duration_x_receipts)_per_mile'] = (df['trip_duration_days'] * df['total_receipts_amount']) / df['miles_traveled'].replace(0, 1)
    #df['receipts_+_(miles_per_day)'] = df['total_receipts_amount'] + (df['miles_traveled'] / df['trip_duration_days'].replace(0, 1))
